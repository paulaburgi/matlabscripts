...
DEM='../dem/stitched_i2.dem';
unwrap='snaphu_mcf'; %icu/snaphu_mcf
filter='0.4';
...
%combine dates
%a=combnk(tt,2); 
a=[ones(length(tt)-1,1).*tt(1) tt(2:end)];
[masterSAR,index]=sort(a(:,1));
slaveSAR=a(index,2);
...
%combine dates
%a=combnk(dates,2);
a=[ones(length(dates)-1,1).*dates(1) dates(2:end)];
[master,index]=sort(a(:,1));
slave=a(index,2);
...
%combine h5 names
ah5=combnk(cskh5,2);
%ah5=[ones(length(cskh5)-1,1).*dates(1) dates(2:end)];
%[masterh5,indexh5]=sort(ah5(:,2));
%slaveh5=ah5(indexh5,1);
masterh5=ah5(:,2);
masterh5=repmat(cskh5(1),length(master),1);
slaveh5 =ah5(:,1);
slaveh5=cskh5(2:end);
...
system('rm run_baseline_1date.sh');


fid3 = fopen('run_ints_1date.sh','a');
fprintf(fid3,'#!/bin/sh');
fprintf(fid3,'\n');
fclose(fid3);
...
for jj=1:(num_ints)
    ...
    fid2 = fopen('run_baseline_1date.sh','a');
    fid3 = fopen('run_ints_1date.sh','a');
...
%             fprintf(fid,strcat('       <property name="posting">',posting,'</property>\n'));
            ...
            fprintf(fid,'	<property name="geocode list">["merged/filt_topophase.unw","merged/filt_topophase.unw.conncomp","merged/los.rdr","merged/topophase.cor"]</property>\n');
...
%print file to run
string=strcat(['./run_isce_1date.sh ',num2str(master(ii)),' ',num2str(slave(ii))]);
fprintf(fid3,string);
fprintf(fid3,'\n');
%end
fclose(fid3);
...
%system('rm -r int* list* junk* mkdir* process* run_baseline_1date.sh* *.ps')

%system('rm int*proc junk* list* process*')

system('chmod 777 run_ints_1date.sh');