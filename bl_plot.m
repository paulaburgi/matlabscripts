...
for i=1:length(sz)
    ...
    d2 = sn(12:17); 
    if ~exist(sn(1:end-5))
        system(sprintf('process_2pass.pl %s raw orbbase', sn)); 
    end
    if ~exist(sprintf('%s/%s_%s_baseline.rsc', sn(1:end-5), d1, d2))
        system(sprintf('process_2pass.pl %s raw orbbase', sn)); 
    end
    cd(sn(1:end-5))
    [~,blc] = system('more *baseline.rsc | grep P_BASELINE_TOP'); 
    bl = str2num(blc(30:end)); 
    bl_all = [bl_all; bl];
    d1_all = [d1_all; d1];
    d2_all = [d2_all; d2];
