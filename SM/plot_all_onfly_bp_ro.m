function [output]= plot_all_onfly_bp_ro(xpt,ypt,plotflag,slcflag)
...
% may storm, october storm dates
ms = datenum('2018/05/24'); 
os = datenum('2018/10/11');
...
if(plotflag)
    ...
    for i=1:length(output)
        plot([output(i).dn],[output(i).mags],cols{k}, 'color',cols{i})
    end
    axis tight
    ax=axis;
    datetick('x','mmmYY')
    ...
    box on; 
    plot([ms ms], [15 90], '--', 'color', 0.7*[0 0 1]); 
    plot([os os], [15 90], '--', 'color', 0.7*[0 0 1]); 
    legend({'VV', 'VH'}, 'location', 'northeast');
    ...
    plot([ms ms], [0 1], '--', 'color', 0.7*[0 0 1]); 
    plot([os os], [0 1], '--', 'color', 0.7*[0 0 1]); 
