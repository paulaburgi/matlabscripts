% invert for rel, perm, roughness terms


pol = '_VV';
decide_ints_stack_ro
   
%bin baselines for later
load baselines.txt
...
% normalize filter func
windx = windx/sum(windx);
windy = windy/sum(windy);

% size of filter func, which is alooks once it is filtered
ry = floor(length(windy)/2);

% dn = datenum of all slc dates
% nd = number of dates
dni   = dn(1:nd-1)+diff(dn)/2;

% time between slc dates
intdt = diff(dn);
...
% make dir and files for the results
adir     = ['/data/pmb229/other/temp_sm_inv/results_dates_bp' pol '/'];
if(~exist(adir,'dir'))
...
cidl     = tril(ones(nd),-1)==1; % lower triangle matrix (0 in top triange, 1 in bottom triangle)
errslope = 0.3; % ??
